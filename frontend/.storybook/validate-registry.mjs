...
const componentsByName = new Map(index.components.map((component) => [component.name, component]));
const errors = [];
const warnings = [];
...
for (const sourcePath of getComponentFiles()) {
  ...
  if (!fs.existsSync(specPath)) {
    errors.push(`${relativePath(sourcePath)} is missing ${path.basename(specPath)}`);
    continue;
  }
  ...
  if (spec.name !== componentName) {
    errors.push(`${relativePath(specPath)} name must match ${componentName}.svelte`);
  }
  for (const token of spec.tokens ?? []) {
    if (!tokenNames.has(token)) {
      errors.push(`${relativePath(specPath)} references unknown token '${token}'`);
    }
  }
  if (spec.status === "figma-linked") {
    if (!spec.figma?.url || !spec.figma?.nodeId) {
      errors.push(`${relativePath(specPath)} is figma-linked but missing figma.url or figma.nodeId`);
    }
    if (spec.tier === "primitive" && (!spec.props || spec.props.length === 0)) {
      errors.push(`${relativePath(specPath)} is a figma-linked primitive with no props[] mapping`);
    }
  }
  if (!fs.existsSync(storyPath)) {
    errors.push(`${relativePath(sourcePath)} is missing ${path.basename(storyPath)}`);
  } else {
    ...
    if (!story.includes(`title: "${expectedTitle}"`)) {
      errors.push(`${relativePath(storyPath)} must declare title: "${expectedTitle}"`);
    }
    if (!story.includes("tags: [\"autodocs\"]")) {
      errors.push(`${relativePath(storyPath)} must include tags: [\"autodocs\"]`);
    }
...
for (const component of index.components) {
  for (const dependency of component.dependencies ?? []) {
    ...
    if (component.tier === "primitive") {
      errors.push(`${component.name} is primitive and must not depend on ${dependency}`);
    }
    if (component.tier === "component" && dep.tier !== "primitive") {
      errors.push(`${component.name} is component and may only depend on primitives (${dependency} is ${dep.tier})`);
    }
  }
  if ((component.tier === "component" || component.tier === "view") && (component.tokens?.length ?? 0) > 6) {
    warnings.push(`${component.name} lists many raw tokens; prefer pushing token ownership into primitives`);
  }
}
...
console.log(`Design-system validation passed: ${index.components.length} components, checklist ${relativePath(checklistPath)}`);
