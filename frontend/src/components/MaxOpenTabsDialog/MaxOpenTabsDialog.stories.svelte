<script module lang="ts">
  ...
  import Component from "./MaxOpenTabsDialog.svelte";
  import StoryRender from "@/stories/StoryRender.svelte";
  import { getStoryArgTypes, getStoryArgs } from "@/stories/fixtures";

  const componentName = "MaxOpenTabsDialog";
  const storyId = "Components/MaxOpenTabsDialog";
  const props: string[] = [];
  const storyArgs = getStoryArgs(storyId, componentName, props);

  const { Story } = defineMeta({
    title: "Components/MaxOpenTabsDialog",
    component: Component,
    tags: ["autodocs"],
