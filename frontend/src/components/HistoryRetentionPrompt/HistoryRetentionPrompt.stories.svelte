<script module lang="ts">
  ...
  import Component from "./HistoryRetentionPrompt.svelte";
  import StoryRender from "@/stories/StoryRender.svelte";
  import { getStoryArgTypes, getStoryArgs } from "@/stories/fixtures";

  const componentName = "HistoryRetentionPrompt";
  const storyId = "Components/HistoryRetentionPrompt";
  const props: string[] = [];
  const storyArgs = getStoryArgs(storyId, componentName, props);

  const { Story } = defineMeta({
    title: "Components/HistoryRetentionPrompt",
    component: Component,
    tags: ["autodocs"],
