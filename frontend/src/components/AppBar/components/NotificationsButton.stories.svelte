<script module lang="ts">
  ...
  import Component from "./NotificationsButton.svelte";
  import StoryRender from "@/stories/StoryRender.svelte";
  import { getStoryArgTypes, getStoryArgs } from "@/stories/fixtures";

  const componentName = "NotificationsButton";
  const storyId = "Components/AppBar/NotificationsButton";
  const props: string[] = [];
  const storyArgs = getStoryArgs(storyId, componentName, props);

  const { Story } = defineMeta({
    title: "Components/AppBar/NotificationsButton",
    component: Component,
    tags: ["autodocs"],
