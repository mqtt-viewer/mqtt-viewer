<script module lang="ts">
  ...
  import Component from "./DropdownCloseOnClick.svelte";
  import StoryRender from "@/stories/StoryRender.svelte";
  import { getStoryArgTypes, getStoryArgs } from "@/stories/fixtures";

  const componentName = "DropdownCloseOnClick";
  const storyId = "Primitives/DropdownMenu/DropdownCloseOnClick";
  const props: string[] = [];
  const storyArgs = getStoryArgs(storyId, componentName, props);

  const { Story } = defineMeta({
    title: "Primitives/DropdownMenu/DropdownCloseOnClick",
    component: Component,
    tags: ["autodocs"],
