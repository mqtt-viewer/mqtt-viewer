<script module lang="ts">
  ...
  import Component from "./MessageStats.svelte";
  import StoryRender from "@/stories/StoryRender.svelte";
  import { getStoryArgTypes, getStoryArgs } from "@/stories/fixtures";

  const componentName = "MessageStats";
  const storyId = "Components/AppBarBottom/MessageStats";
  const props: string[] = [];
  const storyArgs = getStoryArgs(storyId, componentName, props);

  const { Story } = defineMeta({
    title: "Components/AppBarBottom/MessageStats",
    component: Component,
    tags: ["autodocs"],
