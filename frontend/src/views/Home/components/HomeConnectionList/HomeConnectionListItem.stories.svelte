<script module lang="ts">
  ...
  import Component from "./HomeConnectionListItem.svelte";
  import StoryRender from "@/stories/StoryRender.svelte";
  import { getStoryArgTypes, getStoryArgs } from "@/stories/fixtures";

  const componentName = "HomeConnectionListItem";
  const storyId = "Views/Home/HomeConnectionList/HomeConnectionListItem";
  const props: string[] = ["connection"];
  const storyArgs = getStoryArgs(storyId, componentName, props);

  const { Story } = defineMeta({
    title: "Views/Home/HomeConnectionList/HomeConnectionListItem",
    component: Component,
    tags: ["autodocs"],
