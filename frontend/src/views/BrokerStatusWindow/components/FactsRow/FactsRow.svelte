<script lang="ts">
  // One-line broker facts for the broker-status health strip: broker/version,
  ...
  // it shares the strip's wrapping row with the health chips, so it has to move
  // onto the next line whole and ellipsise on a window narrower than the line
  // itself. Only a single text node does that.
