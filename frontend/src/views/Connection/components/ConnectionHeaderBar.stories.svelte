<script module lang="ts">
  ...
  import Component from "./ConnectionHeaderBar.svelte";
  import StoryRender from "@/stories/StoryRender.svelte";
  import { getStoryArgTypes, getStoryArgs } from "@/stories/fixtures";

  const componentName = "ConnectionHeaderBar";
  const storyId = "Views/Connection/ConnectionHeaderBar";
  const props: string[] = ["connection","connectionIsValid"];
  const storyArgs = getStoryArgs(storyId, componentName, props);

  const { Story } = defineMeta({
    title: "Views/Connection/ConnectionHeaderBar",
    component: Component,
    tags: ["autodocs"],
