<script module lang="ts">
  ...
  import Component from "./SubscriptionsForm.svelte";
  import StoryRender from "@/stories/StoryRender.svelte";
  import { getStoryArgTypes, getStoryArgs } from "@/stories/fixtures";

  const componentName = "SubscriptionsForm";
  const storyId = "Views/Connection/ConnectionDetailsView/SubscriptionsForm";
  const props: string[] = ["connection"];
  const storyArgs = getStoryArgs(storyId, componentName, props);

  const { Story } = defineMeta({
    title: "Views/Connection/ConnectionDetailsView/SubscriptionsForm",
    component: Component,
    tags: ["autodocs"],
