<script module lang="ts">
  ...
  import Component from "./SidebarTopBar.svelte";
  import StoryRender from "@/stories/StoryRender.svelte";
  import { getStoryArgTypes, getStoryArgs } from "@/stories/fixtures";

  const componentName = "SidebarTopBar";
  const storyId = "Views/Connection/DataView/Sidebar/SidebarTopBar";
  const props: string[] = ["connection","collapseSidebar","onSearch"];
  const storyArgs = getStoryArgs(storyId, componentName, props);

  const { Story } = defineMeta({
    title: "Views/Connection/DataView/Sidebar/SidebarTopBar",
    component: Component,
    tags: ["autodocs"],
