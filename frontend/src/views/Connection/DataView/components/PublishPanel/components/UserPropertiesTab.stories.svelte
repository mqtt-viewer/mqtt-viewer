<script module lang="ts">
  ...
  import Component from "./UserPropertiesTab.svelte";
  import StoryRender from "@/stories/StoryRender.svelte";
  import { getStoryArgTypes, getStoryArgs } from "@/stories/fixtures";

  const componentName = "UserPropertiesTab";
  const storyId = "Views/Connection/DataView/PublishPanel/UserPropertiesTab";
  const props: string[] = ["userProperties"];
  const storyArgs = getStoryArgs(storyId, componentName, props);

  const { Story } = defineMeta({
    title: "Views/Connection/DataView/PublishPanel/UserPropertiesTab",
    component: Component,
    tags: ["autodocs"],
