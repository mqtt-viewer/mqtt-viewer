<script module lang="ts">
  ...
  import Component from "./BigAddConnectionButton.svelte";
  import StoryRender from "@/stories/StoryRender.svelte";
  import { getStoryArgTypes, getStoryArgs } from "@/stories/fixtures";

  const componentName = "BigAddConnectionButton";
  const storyId = "Views/NewTab/BigAddConnectionButton";
  const props: string[] = ["onClick"];
  const storyArgs = getStoryArgs(storyId, componentName, props);

  const { Story } = defineMeta({
    title: "Views/NewTab/BigAddConnectionButton",
    component: Component,
    tags: ["autodocs"],
